...
export const list = async (req: Request, res: Response) => {
	try {
		const dataList = await voteModel.getMany(['vote', 'created_at'])
...
export const add = async (req: customRequest, res: Response) => {
	let transaction
	try {
		const { user_id, question_id, answer_id, vote } = req.body
		const body = {
			user_id: user_id,
		...
		if (vote == 'true') {
			...
			const addScore = await scoreModel.addScore({
		...
		const totalScore = await scoreModel.totalScore({ user_id: user.user_id })
		...
		const addVote: any = await voteModel.addVote(body)
